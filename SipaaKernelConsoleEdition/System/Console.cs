...
using PrismGL2D;
using SipaaKernelConsoleEdition.GraphicsExtensions;

namespace SipaaKernelConsoleEdition.System
{
    public struct Cell
    {
        public char? Char;
    ...
    public class GUI
    {
        public void DrawFilledRectangle(Color pen, int x_start, int y_start, int width, int height)
        {
            ...
            {
                Kernel.canvas.DrawLine(x_start, y, x_start + width + 1, y, pen);
            }
        }

        public void WriteByte(char ch, int mX, int mY, Color pen)
        {
            Kernel.canvas.DrawPSFChar(mX, mY, (char)ch, Kernel.font, pen);
        }
        ...
        public void SetCursorPos(int mX, int mY)
        {
            ...
            {
                DrawFilledRectangle(Kernel.Console.ForegroundPen, (int)0 + mX * Kernel.font.Width,
                    (int)0 + mY * Kernel.font.Height + Kernel.font.Height, 8, 4);
            }
        }
    }
    public class Console
    {
        ...
        public void Update()
        {
            ...
            Kernel.canvas.DrawFilledRectangle((int)0, (int)0, Kernel.canvas.Width, Kernel.canvas.Height, 0, Color.Black);
        ...
        void DrawTerminal()
        {
            for (int i = 0; i < mRows; i++)
            {
                for (int j = 0; j < mCols; j++)
                {
                    ...
                    Graphics.WriteByte((char)Text[i][j].Char, (int)0 + j * Kernel.font.Width, (int)0+ i * Kernel.font.Height, Text[i][j].Colour);
                }
            }
            ...
            if (Command.Length > 0)
            {
                ...
                {
                    Graphics.WriteByte(Command[i], (int)0 + ((baseX + i) * Kernel.font.Width), (int)0 + mY * Kernel.font.Height, ForegroundPen);
                }
            }
        }
